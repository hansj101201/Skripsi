...
@push('js')
    ...
    <script>
                ...
                updateGudangOptions(urlGudangAsalActive, function() {
                    updateGudangTujuanOptions(urlGudangTujuanAll, function() {
                        ...
                        if (!$('#tanggal').val()) {
                            ...
                            updateNomorPo(urlNoPoActive, requestBody);
                        }
                        enableDatepicker(); // Set nilai input dengan ID 'tanggal' menjadi tanggal yang telah diformat
        ...
        $('#tanggal').change(function() {
            ...
            updateNomorPo("{{ url('transaksi/pengeluaran/getPermintaanActive') }}", requestBody);
            mode = 'add';
        });
            ...
            if (!tanggal) {
                ...
                if (mode === 'add' || addMode) {
                    ...
                    getData(kode, tanggal, gudang, function() {
                        $('#editMode').val('add');
                        if (qtykirim > parseFloat($('#saldo').val().replace(
                                /[^\d]/g, ''))) {
                            $('#qtykirim').val(formatHarga(parseFloat($('#saldo')
                                .val().replace(
                                    /[^\d]/g, ''))));
                        } else {
        ...
        $('#confirmDeleteButton').on('click', function() {
            $.ajax({
                method: 'DELETE',
                url: "{{ url('transaksi/pengeluaran/delete') }}/" + bukti + "/" +
                    periode,
                data: {
                    '_token': '{{ csrf_token() }}',
                },
                success: function(response) {
                    $('#deleteDataModal').modal(
                        'hide'); // Correct the selector here
                    $('.modal-backdrop').remove();
                    toastr.success(response.message);
                    table.draw();
                },
